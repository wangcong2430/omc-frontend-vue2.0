      ...
      <table class="table-default">
        ...
        <tbody>
          <tr v-for="(list, i) in tableData" :key="`${list.id}-${i}`">
            ...
            <td>{{ list.create_time | parseTime('{y}-{m}-{d} {h}:{i}') }}</td>
            <td>
            ...
            <td>
              <ul class="omc-inline-actions">
                <li v-permission="['button/workbench_planning_audit']" v-todo @click="approvalBudget(list)">
                  <a href="javascript:;" class="btn-second">通过</a>
                </li>
                <li v-permission="['button/workbench_planning_reject']" v-todo @click="rejectBudget(list)">
                  <a href="javascript:;" class="btn-second">驳回</a>
                </li>
                <li v-if="list.budget_type !== 'outsource'" @click="showBudgetDetail(list)">
...
import BudgetModal from '@/views/budget/planning/budget-list/budget-modal/index'
import workbenchMixins from './mixins/index'
import { getBacklogList, completeBacklog } from '@/api/workbench/index'
import { getBudgetRdInfo } from '@/api/budget/research'
import { getBudgetCpInfo } from '@/api/budget/content'
...
export default {
  ...
  methods: {
    getPlanningList() {
      getBacklogList(this.page, this.pageSize, 1, this.workbenchType).then(res => {
        const { data } = res
    ...
    rejectAll() {
      if (!checkPermission(['button/workbench_planning_reject'])) {
    ...
    approvalAll() {
      if (!checkPermission(['button/workbench_planning_audit'])) {
    ...
    async showBudgetDetail(list) {
      ...
      const promise = new Promise((resolve, reject) => {
        if (list.budget_type === 'rd') {
          getBudgetRdInfo(list.id).then(res => {
            const { data } = res
            resolve(data)
          }).catch(err => reject(err))
        }
        if (list.budget_type === 'cp') {
          getBudgetCpInfo(list.id).then(res => {
            const { data } = res
            resolve(data)
          }).catch(err => reject(err))
        }
    ...
    async onSubmitApproval(values) {
      if (values.operate_status === 1 && !checkPermission(['button/workbench_planning_audit'])) {
        return this.$message.info('您没有权限进行审批通过，请联系管理员开通权限！')
      }
      if (values.operate_status === 2 && !checkPermission(['button/workbench_planning_reject'])) {
